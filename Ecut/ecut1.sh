...
# cubic crystals scanning from 50 - 100 Ry in steps of 5 Ry.

rm -rf etot.dat stress.dat
for k in {0..10}    # k = (100-50)/5
  do
    ecut=`echo "50.0+$k*5.0" | bc -l`
cat > scf.in << EOF
&control
    calculation='scf'
    restart_mode='from_scratch'
    pseudo_dir = '/home/hp/Desktop/test
    outdir='./'
    disk_io='none'
    tstress=.true.
    tprnfor=.true.
...
K_POINTS {automatic}
 8 8 8 1 1 1
EOF
mpirun -np 4 pw.x  -inp scf.in > scf.out
